<?php
    ...
    $sql = "UPDATE prog_studi SET kode_prodi='$id',prodi='$prodi',fakultas='$fakultas',akreditasi='$akreditasi' where kode_prodi=$id";
    $query = mysqli_query($conn,$sql);
    
    if($query){
    echo '<script>alert("Data Berhasil Diubah!");window.location.href="adm_prodi.php"</script>';
    }else{
      echo '<script>alert("Data Gagal Diubah!");window.location.href="edit_prodi.php"</script>';
      }
    }
  ?>
<!DOCTYPE html>
          ...
          <form action="" method="POST">
            <?php 
              ...
              if(isset($_GET['kode_prodi'])){
                $id = $_GET['kode_prodi'];
                $sql = "SELECT * FROM prog_studi WHERE kode_prodi=$id";
                $query = mysqli_query($conn,$sql);
                while ($data = mysqli_fetch_array($query)){
            ?>
            <label for="kode">Kode</label>
            <div class="form-floating mb-3">
              <input type="text" class="form-control" name="kode" value="<?php echo $data['kode_prodi']?>" required id="kode" autocomplete="off">
            </div>

            <label for="prodi">Nama Program Studi</label>
            <div class="form-floating mb-3">
              <input type="text" class="form-control" name="prodi" value="<?php echo $data['prodi']?>" required id="prodi" autocomplete="off">
            </div>

            <label for="fakultas">Fakultas</label>
            <div class="form-floating mb-3">
              <input type="text" class="form-control" name="fakultas" value="<?php echo $data['fakultas']?>" required id="fakultas" autocomplete="off">
            </div>

            <label for="akreditasi">Akreditasi</label>
            <div class="form-floating mb-3">
              <input type="text" class="form-control" name="akreditasi" value="<?php echo $data['akreditasi']?>" required id="akreditasi" autocomplete="off">
            </div>
            <br> 
            <?php
                  }
  ...
  <script>
    $(document).ready(function() {
      $('.btn.btn-secondary').click(function() {
        document.location.href='adm_prodi.php';
      })
    })
  </script>
</body>
